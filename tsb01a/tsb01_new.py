...
#       ### read ADC here (before RESET)
    ...
    def sel_all_get_adc(self, ext=True, timeout=1000):  # ## timeout[ms]
        ...
        val1 = np.bitwise_and(nmdata, 0x00003fff)  # select data from ADC1
        vals = np.right_shift(np.bitwise_and(nmdata, 0x10000000), 28)  # move 28th bit of data to bit0
        valc = np.right_shift(np.bitwise_and(nmdata, 0x60000000), 29)  # move 29th and 30th bit of data to bit0 and bit1 
       
        if(not single):
            val0 = np.right_shift(np.bitwise_and(nmdata, 0x0fffc000), 14) # select data from ADC0 and shift to bit0 - bit 13
    ...
    def trigger(self, th, period, save=True):
        ...
        logging.info("trigger th:%d %d" % (th[0], th[1]))
        return dat
